...
class AlphaDIAFragTableProcessor:
    ...
    def __init__(self, fragment_matrix_file: str):
        ...
        self.ml_info_file = aq_utils.get_progress_folder_filename(fragment_matrix_file, ".ml_info_table.tsv") #the ml info table is derived from the precursors.tsv, but for the fragion matrix. This is why the filename must match the fragment matrix file
    ...
    def write_processed_files(self) -> None:
        """
        Write both the processed fragment matrix and ML info table to files.
        """
        # Write processed fragment matrix
        if self.processed_fragment_matrix is not None:
            self.processed_fragment_matrix.to_csv(
                self.input_file_reformat,
                sep="\t",
                index=False
            )
        else:
            raise ValueError("Processed fragment matrix not created")

        # Write ML info table
        if self.ml_info_df is not None:
            self.ml_info_df.to_csv(
                self.ml_info_file,
                sep="\t",
                index=False
            )
        else:
            raise ValueError("ML info table not created")