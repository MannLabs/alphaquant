...
__all__ = ['get_condpairname', 'get_middle_elem', 'get_nonna_array', 'get_non_nas_from_pd_df', 'get_ionints_from_pd_df',
           'invert_dictionary', 'ion_hierarchy', 'get_relevant_columns', 'get_relevant_columns_config_dict',
           'retrieve_configuration', 'get_config_columns', 'get_config_columns_w_hierarchy', 'load_config',
           'get_type2relevant_cols', 'filter_input', 'merge_protein_and_ion_cols', 'merge_protein_cols_and_ion_dict',
           'reformat_longtable_according_to_config', 'reformat_longtable_according_to_config_new',
           'read_wideformat_table', 'read_mq_peptides_table', 'check_for_processed_runs_in_results_folder',
           'import_data', 'get_samplenames', 'load_samplemap', 'prepare_loaded_tables']
...
def get_relevant_columns_config_dict(config_dict):
    filtcols = []
    dict_ioncols = []
    for filtconf in config_dict.get('filters', {}).values():
        filtcols.append(filtconf.get('param'))

    for headr in config_dict.get('ion_hierarchy', {}).values():
        dict_ioncols.append(headr)

    relevant_cols = config_dict.get("protein_cols") + config_dict.get("ion_cols", []) + [config_dict.get("sample_ID")] + [config_dict.get("quant_ID")] + filtcols + dict_ioncols
    relevant_cols = list(set(relevant_cols)) # to remove possible redudancies
    return relevant_cols
...
def get_config_columns_w_hierarchy(config_dict):
    protein_cols = config_dict.get("protein_cols")
    ion_cols = config_dict.get("ion_cols")
    sample_ID = config_dict.get("sample_ID")
    quant_ID = config_dict.get("quant_ID")
