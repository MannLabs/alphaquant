...
__all__ = ['AlphaPeptColorMap', 'IonPlotColorGetter', 'plot_pvals', 'plot_bgdist', 'tranform_fc2count_to_fc_space',
           ...
           'ProteinIntensityDataFrameGetter']
...
class IonFoldChangePlotter():
    def __init__(self, melted_df, condpair, specified_level = 'mod_seq_charge', property_column = "predscore", is_included_column="is_included"):
        self._melted_df = melted_df
        self._condpair = condpair
        self._specified_level = specified_level
        self._property_column = property_column
        self._is_included_column = is_included_column
        self.__get_precursors_and_fcs_from_melted_df()

    def plot_ion_selection_overview(self):
        fig, axs = plt.subplots(2, 2,figsize = self.__get_fig_width())
        colorgetter = IonPlotColorGetter(melted_df = self._melted_df, property_column=self._property_column, ion_name_column=self._specified_level, is_included_column=self._is_included_column)
    ...
    def __get_precursors_and_fcs_from_melted_df(self):
        multiindex_df = self._melted_df.set_index(["condition", 'ion'])
        ...
        for ion in df_c1.index.intersection(df_c2.index):
            ...
            precursor = df_c1.loc[ion][self._specified_level][0]
...
class ProteinIntensityDataFrameGetter():
    ...
    def __init__(self, condpair, results_dir, samplemap, ion_header = 'ion', specified_level =  "mod_seq_charge"):
        self._ion_header = ion_header
        self._specified_level = specified_level
        self.__load_relevant_data(condpair, results_dir, samplemap)

    def get_melted_protein_ion_intensity_table(self, protein):

        value_vars = set.intersection(set(self.normed_df.columns), set(self._sample2cond.keys()))
        protein_df = self.normed_df.xs(protein, level = 0)
        df_melted = pd.melt(protein_df.reset_index(), value_vars= value_vars, id_vars=[self._ion_header], value_name="intensity", var_name="sample")
        df_melted["condition"] = [self._sample2cond.get(x) for x in df_melted["sample"]]

        #if ion clustering has been performed, add cluster information
        if self._condpair_root_node != None:
            df_melted = self.__annotate_cluster_information(df_melted, protein)

        return df_melted

    def get_protein_diffresults(self, protein):
        return self.diffresults_df.loc[protein]

    def __load_relevant_data(self, condpair, results_dir, samplemap):
        cond1 = condpair[0]
        cond2 = condpair[1]
        ...
        self._condpair_root_node = aqutils.read_condpair_tree(cond1, cond2, results_folder= results_dir)
        samplemap_df = aqutils.load_samplemap(samplemap)
        self._sample2cond = self.__get_sample2cond(samplemap_df)



    def __annotate_cluster_information(self, df_melted, protein): #mod_seq_charge
        ...
        #annotate properties
        ion2is_included = self.__get_ionmap_dict(protein_node, self.__assign_if_node_at_level_is_included, self._specified_level)
        df_melted["is_included"] = [ion2is_included.get(x) for x in df_melted["ion"]]

        ion2predscore = self.__get_ionmap_dict(protein_node, self.__assign_predscore_of_node_at_level, self._specified_level)
        df_melted["predscore"] = [ion2predscore.get(x) for x in df_melted["ion"]]

        ion2precursor = self.__get_ionmap_dict(protein_node, self.__assign_name_of_node_at_level, self._specified_level)
        df_melted[self._specified_level] = [ion2precursor.get(x) for x in df_melted["ion"]]
