...
__all__ = ['DifferentialIon', 'calc_diffreg_peptide', 'calc_outlier_scaling_factor']
...
class DifferentialIon():
    ...
    def __init__(self,noNanvals_from, noNanvals_to, diffDist):
        self.usable = False
...
def calc_outlier_scaling_factor(noNanvals_from, noNanvals_to, diffDist):
    ...
    between_rep_SD_from = math.sqrt(sum(np.square(noNanvals_from-median_from))) if len(noNanvals_from)>1 else sd_from
    between_rep_SD_to = math.sqrt(sum(np.square(noNanvals_to-median_to))) if len(noNanvals_to)>1 else sd_to
