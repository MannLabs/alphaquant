...
__all__ = ['plot_pvals', 'plot_bgdist', 'tranform_fc2count_to_fc_space', 'plot_betweencond_fcs', 'scatter_df_columns',
           'plot_cumhist_dfcols', 'compare_peptid_protein_overlaps', 'plot_fold_change', 'volcano_plot']
...
def volcano_plot(result_df):
    result_df["fdr"] = result_df["fdr"].replace(0, np.min(result_df["fdr"].replace(0, 1.0)))
    plt.scatter(result_df["fc"],-np.log10(result_df["fdr"]),s=10, c='grey', alpha = 0.7)
    plt.xlabel('log2 FC',fontsize = 14)
    plt.ylabel('-log10 FDR',fontsize = 14)
    plt.ylim(0,max(-np.log10(result_df["fdr"]))+0.5)
    maxfc = max(abs(result_df["fc"]))+0.5
